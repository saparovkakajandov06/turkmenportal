    ...
    <?php if (Yii::app()->controller->isMobile()) { ?>
        <header class="entry-header">
            <h4 class="entry-title">
                <?php
                echo CHtml::link($title, $url, array('alt' => $title, 'rel' => 'bookmark'));
                ?>
            </h4>
        ...
        <?php
        $thumb = $data->getThumbPath(450, 450, 'w', true);
        if (isset($thumb) && strlen(trim($thumb)) > 0) { ?>
            <figure class="entry-thumbnail  mobile-responsive">
                <?php echo CHtml::link(CHtml::image($thumb, $title), $url, array('class' => "thumb")); ?>
            </figure>
        <?php } ?>
    <?php } else {
