...
//		Yii::app()->bootstrap->registerScrollSpy();		
    ...
    public function run() {
        ...
        <?php
        echo CHtml::tag("div", array("class" => "section bg-primary carousel carousel slide", "id" => $this->id));
        ?>
        <!-- Carousel items -->
        <?php
        ...
        foreach ($dataArray as $i => $data) {
            if ($i == 0) {
                echo CHtml::tag("div", array("class" => "active item row entries"));
            } elseif ($n == $this->coloumCount) {
                echo CHtml::tag("div", array("class" => "item row entries"));
                $n = 0;
            }
