...
class AweCrudCode extends CrudCode {
    ...
    public $defaultLayout = '//layouts/column2_admin';
    ...
    public function getColumnExceptIdentificationString($relatedModelName) {
        $final_array_sting='';
        $related_table=
        $relatedModelClassTableSchema=CActiveRecord::model($relatedModelName)->getTableSchema();
        $relatedModelIdentificationColumn=Awecms::getPrimaryKeyColumn(CActiveRecord::model($relatedModelName));
        
        $columns_name = array_map('self::getName', $relatedModelClassTableSchema->columns);
        
        foreach ($columns_name as $column_name) {
            if (!empty($relatedModelIdentificationColumn))
                if($relatedModelIdentificationColumn==$column_name)
                    continue;
            $final_array_sting.="'$column_name',";
        }
        return $final_array_sting;
    }
    
    
    public function getRelationsAsArrayString() {
        $final_array_sting='';
        $relations_names =array();
        $relations=$this->getRelations();
        foreach ($relations as $relName => $value) {
            $relations_names[]=$relName;
        }
        
        foreach ($relations_names as $rel_name) {
            $final_array_sting.="'$rel_name',";
        }
        return $final_array_sting;
    }
...
//array_keys(CHtml::listData( $model->categories, 'id' , 'id')
        ...
        $relatedModelPrimaryKey=Awecms::getPrimaryKeyColumn(CActiveRecord::model($relation[1]));

//        $str .= "<?php echo CHtml::checkBoxList('{$modelClass}[{$relatedModelClass}]', array_map('Awecms::getPrimaryKeyColumn',\$model->{$relatedModelClass}),
        $str .= "<?php echo CHtml::checkBoxList('{$modelClass}[{$relatedModelClass}]', array_keys(CHtml::listData(\$model->{$relatedModelClass},'".$relatedModelPrimaryKey."','".$relatedModelPrimaryKey."')),
