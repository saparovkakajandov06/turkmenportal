...
class DCategoryTreeBehavior extends DCategoryBehavior
{
    ...
    public function findByPath($path)
    {
        ...
        if (count($domens)==1) {
            ...
            $criteria->mergeWith(array(
                'condition'=>'t.' .$this->aliasAttribute . '=:alias and parent_id is null',
                'params'=>array(':alias'=>$domens[0])
            ));
    ...
    public function getPath($separator='/',$with_prefix=false)
    {
        ...
        $this->urlPrefixValue=trim($this->getOwner()->{$this->urlPrefix},$separator);
        ...
        while ($i-- && $this->cached($category)->{$this->parentRelation}){
            ...
            $this->urlPrefixValue=trim($category->{$this->urlPrefix},$separator);
        }
    ...
    public function translatePath($path,$lang=null){
        $domens = explode('/', trim($path, '/'));
        $domens_tr =array();
        $criteria = $this->getOwnerCriteria();
        $lang=isset($lang) ? $lang : Yii::app()->getLanguage();

        if(isset($domens) && is_array($domens) && count($domens)>0){
            foreach ($domens as $alias){
                $criteria->mergeWith(array(
                    'condition'=>'t.alias_ru=:alias OR t.alias_tm=:alias OR t.alias_en=:alias',
                    'params'=>array(
                        ':alias'=>$alias,
                    )
                ));
                $model=$this->cached($this->getOwner())->find($criteria);
                if(isset($model))
                    $domens_tr[]=$model->{'alias_' . $lang};
            }

            if(isset($domens_tr) && is_array($domens_tr)){
                $path=implode('/',$domens_tr);
            }
        }
