...
class RController extends CController {
    ...
    public $page_url;
    public $page_image;
    public $profile_menu = array();
    public $page_links = array();
    public $meta_description = '';
    public $meta_keyword = '';
    public $is_inner_breadcrumb = false;
    ...
    public function checkRightsFromUrl($url) {
        $normalized_url = CHtml::normalizeUrl($url);
        $normalized_url = split("r=", $normalized_url);
        $role = "";
        if (is_array($normalized_url) && count($normalized_url) > 1) {
            $normalized_url = $normalized_url[1];
            $route = split("&", $normalized_url);
            if (is_array($route) && count($route) > 0)
                $route = $route[0];
            $path = explode("/", $route);
            if (isset ($path) && is_array($path)) {
                foreach ($path as $key => $w) {
                    $path[$key] = ucfirst($w);
                }
            }
            $role = implode(".", $path);
        ...
        if (isset ($role) && strlen(trim($role)) > 0)
