<?php if(isset ($this->show_header) && $this->show_header==true) { ?>
    <div class="row box_header_index">
        <?php echo CHtml::link($this->categoryModel->name, Yii::app()->createUrl('//catalog/category/',array('category_id'=>$this->categoryModel->id)), array('class' => "mini headerColor")); ?>
    </div>
<?php }?>
...
if(isset ($this->show_sub_header) && $this->show_sub_header==true) { ?>
    <div class="row sub_header_wrapper">
        <div class="">
            <?php echo CHtml::link($this->categoryModel->name, $this->categoryModel->url, array('class' => "subHeaderColor")); ?>
...
//                    'maxButtonCount' => $this->maxButtonCount,
                ...
                'columns' => array(
                    array(
                        'name' => 'documents.name',
                        'value' => 'CHtml::link(CHtml::image($data->getThumbPath(50,50,"auto")),Yii::app()->createUrl("//catalog/view",array("id"=>$data->id)))',
                        'type'=>'raw',
                        'htmlOptions' => array('style' => 'width:60px; padding-right:5px;')
                    ),
                    array(
                        'name' => 'title',
                        'value' => 'CHtml::link($data->title,Yii::app()->createUrl("//catalog/view",array("id"=>$data->id)))',
                        'type'=>'raw',
